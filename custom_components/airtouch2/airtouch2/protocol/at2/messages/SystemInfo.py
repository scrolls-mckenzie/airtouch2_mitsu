...
class AcInfo:
    ...
    def parse(ac_number: int, ac_status1: int, ac_error_code: int, ac_status2: int, ac_mode: int, ac_fan_speed: int,
              ac_set_temp: int, ac_measured_temp: int, ac_brand: int, ac_gateway_id: int, ac_name: bytes) -> Optional[AcInfo]:
        ...
        # Handle AC mode - support standard modes and Mitsubishi-specific modes
        try:
            mode = ACMode(ac_mode)
        except ValueError:
